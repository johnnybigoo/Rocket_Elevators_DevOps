namespace :db do
    ...
    task :fact_quote do
        ActiveRecord::Base.establish_connection(:adapter  => "mysql2",:host => "localhost",:username => "root",:password => "mysql1234",:database => "rocket_elevators_devops_development")
        row = ActiveRecord::Base.connection.execute('SELECT q.id, q.created_at ,q.compagnyName ,q.email ,q.numElevator 
                                                    FROM quotes q; ')
        ActiveRecord::Base.establish_connection(:adapter  => "postgresql",:host => "localhost",:username => "postgres",:password => "allo1392",:database => "data_warehouse_devops_development")
        row.each do |val|
