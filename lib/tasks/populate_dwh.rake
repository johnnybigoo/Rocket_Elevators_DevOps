namespace :db do
    ...
    task :fact_quote do
        ActiveRecord::Base.establish_connection(:adapter  => "mysql2",:host => ENV["MYSQL_DATABASE_HOST"],:username => ENV["MYSQL_DATABASE_USER"],:password => ENV["MYSQL_DATABASE_PASSWORD"],:database => ENV["MYSQL_DATABASE_DBNAME"])
        row = ActiveRecord::Base.connection.execute('SELECT q.id, q.created_at ,q.compagnyName ,q.email ,q.numElevator 
                                                    FROM quotes q; ')
        ActiveRecord::Base.establish_connection(:adapter  => "postgresql",:host => ENV["POSTGRESQL_DATABASE_HOST"],:username => ENV["POSTGRESQL_DATABASE_USER"],:password => ENV["POSTGRESQL_DATABASE_PASSWORD"],:database => ENV["POSTGRESQL_DATABASE_DBNAME"])
        row.each do |val|
